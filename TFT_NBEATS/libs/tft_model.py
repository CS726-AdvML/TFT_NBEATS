...
class TemporalFusionTransformer(object):
    ...
    def _build_base_graph(self, all_inputs, archi_type):
        ...
        p_prime = self.hidden_layer_size
        def trend_layer(transformer_layer, all_inputs, attention_components):
            p = p_prime
            print("I am trend layer")
            H = transformer_layer.shape[2]
            print(H)
            t_f = tf.cast(tf.range(0, H), dtype=tf.float32) / (H)
            T_ = tf.transpose(tf.map_fn(lambda p: t_f ** p, tf.range(p, dtype=tf.float32)))
            transformer_layer = tf.tensordot(transformer_layer, T_, axes=1)
            return transformer_layer, all_inputs, attention_components


        def seasonal_layer(transformer_layer, all_inputs, attention_components):
            p = p_prime/2
            print("I am sesonal layer layer")
            H = transformer_layer.shape[2]
            print(H)
            t_f = tf.cast(tf.range(0, H), dtype=tf.float32) / (H)
            s1 = tf.map_fn(lambda p_: tf.math.cos(2 * np.pi * p_ * t_f), tf.range(p, dtype=tf.float32))
            s2 = tf.map_fn(lambda p_: tf.math.sin(2 * np.pi * p_ * t_f), tf.range(p, dtype=tf.float32))
            S = tf.transpose(tf.concat([s1, s2], axis=0))
            transformer_layer = tf.tensordot(transformer_layer, S, axes=1)
            return transformer_layer, all_inputs, attention_components

        if(archi_type == "trend"):
            return trend_layer(transformer_layer, all_inputs, attention_components)
        elif(archi_type == "seasonal"):
            return seasonal_layer(transformer_layer, all_inputs, attention_components)
        else:
            return transformer_layer, all_inputs, attention_components
    ...
    def build_model(self):
        ...
        with tf.compat.v1.variable_scope(self.name):
            ...
            transformer_layer1, all_inputs1, attention_components1 \
                = self._build_base_graph(all_inputs, "trend")
            transformer_layer2, all_inputs2, attention_components2 \
                = self._build_base_graph(all_inputs, "seasonal")  
            transformer_layer3, all_inputs3, attention_components \
                = self._build_base_graph(all_inputs, "nothing")
            transformer_layer = transformer_layer1 + transformer_layer2 + transformer_layer3
